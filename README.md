...
> ⚠️ **v0.x 免责声明：** 在 1.0.0 正式版到来之前，ASSCOR 可能出现破坏性变更（配置、CLI、接口、数据格式等），市场定位甚至产品领域也可能调整。作者不对任何 0.x 版本提供稳定性或兼容性承诺——请基于当前版本自行评估使用风险。
...
`main` 分支始终为可构建、可测试、可发布状态（CI 全绿）；任何实验性方向（如 `ASSCOR-Research-Core` 研究分支的拓扑/传播层研究）不进 `main`，以可选模块/扩展包形式存在。
