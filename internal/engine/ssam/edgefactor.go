...
// newSynthesizePlan 计算合成计划（V/G/C 专用；同时是在线**可执行性**的装配期闸门）。
//
// 前置条件：调用方已提前处理另外两种模型 —— **legacy**（M0：与"未启用"共用内仓默认的逐次
// 相乘路径、零注册，不需要合成计划）与 **chain**（在线不可执行，见下）。
//
...
// 同款纪律：能装配出来的东西必须真的能算。
//
// V/G 要求「请求的每个域都有 λ_d」（内仓 Synthesize 的 fail-fast），因此请求域
// = DefaultDomains ∩ p.Lambda：**未配置 λ 的域不做域级修正**（不是整次失败后静默退化）。
// 一个 λ 都没配的 V/G 模型永远产生不出任何修正 ⇒ 报错，由装配层拒绝装配并保持默认路径
// ——否则会出现「戳记写着 graph、评分却分毫未变」的假溯源。
//
// 第三条错误分支（"模型没有在线合成计划"）是**穷尽性兜底**：`Params.Validate` 只放行
// legacy/vector/graph/chain 四个模型，前两者已由调用方处理，故它不可达，但保留它可以让
// 本函数对未来新增的模型保持 fail-fast（而不是静默返回一个空计划）。
