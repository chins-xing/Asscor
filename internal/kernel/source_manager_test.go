...
func TestSourceManagerModule_DeploySource(t *testing.T) {
	...
	cfg := SourceConfig{Settings: map[string]string{"target_images": "nginx:latest"}}
...
func TestSourceManagerModule_DeployDuplicate(t *testing.T) {
	...
	spec := SourceSpec{ID: "trivy", Name: "Trivy", Category: SourceCategoryScanner, Priority: SourcePriorityP0, Version: "1.0"}
	m.DeploySource(context.Background(), spec, SourceConfig{})

	err := m.DeploySource(context.Background(), spec, SourceConfig{})
	if err == nil {
...
func TestSourceManagerModule_EnableDisable(t *testing.T) {
	...
	spec := SourceSpec{ID: "nuclei", Name: "Nuclei", Category: SourceCategoryScanner, Priority: SourcePriorityP0, Version: "3.0"}
	m.DeploySource(context.Background(), spec, SourceConfig{})
	...
	if status.State != SourceStateEnabled {
	...
	if status.State != SourceStateDisabled {
...
func TestSourceManagerModule_EnableAlreadyEnabled(t *testing.T) {
	...
	spec := SourceSpec{ID: "lynis", Name: "Lynis", Category: SourceCategoryScanner, Priority: SourcePriorityP0, Version: "3.0"}
	m.DeploySource(context.Background(), spec, SourceConfig{})
...
func TestSourceManagerModule_UpdateSource(t *testing.T) {
	...
	spec := SourceSpec{ID: "trivy", Name: "Trivy", Category: SourceCategoryScanner, Priority: SourcePriorityP0, Version: "0.49.0"}
	m.DeploySource(context.Background(), spec, SourceConfig{})
...
func TestSourceManagerModule_UninstallSource(t *testing.T) {
	...
	spec := SourceSpec{ID: "aide", Name: "AIDE", Category: SourceCategoryScanner, Priority: SourcePriorityP2, Version: "1.0"}
	m.DeploySource(context.Background(), spec, SourceConfig{})
...
func TestSourceManagerModule_ConfigureSource(t *testing.T) {
	...
	spec := SourceSpec{ID: "netbox", Name: "NetBox", Category: SourceCategoryManagement, Priority: SourcePriorityP0, Version: "3.5"}
	m.DeploySource(context.Background(), spec, SourceConfig{})

	newCfg := SourceConfig{Settings: map[string]string{
...
func TestSourceManagerModule_ListSources(t *testing.T) {
	...
	m.DeploySource(context.Background(), SourceSpec{ID: "nuclei", Name: "Nuclei", Category: SourceCategoryScanner, Priority: SourcePriorityP0, Version: "3.0"}, SourceConfig{})
	...
	scanners := m.ListSources(SourceCategoryScanner)
	if len(scanners) != 2 {
		t.Errorf("expected 2 scanners, got %d", len(scanners))
	}

	mgmt := m.ListSources(SourceCategoryManagement)
	if len(mgmt) != 1 {
...
func TestSourceManagerModule_DeployValidation(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:    "empty id",
			spec:    SourceSpec{Name: "Test", Category: SourceCategoryScanner, Priority: SourcePriorityP0},
			wantErr: true,
		},
		{
			name:    "empty name",
			spec:    SourceSpec{ID: "test", Category: SourceCategoryScanner, Priority: SourcePriorityP0},
			wantErr: true,
		},
		{
			name:    "invalid category",
			spec:    SourceSpec{ID: "test", Name: "Test", Category: "invalid", Priority: SourcePriorityP0},
			wantErr: true,
		},
		{
			name:    "invalid priority",
			spec:    SourceSpec{ID: "test", Name: "Test", Category: SourceCategoryScanner, Priority: "P9"},
			wantErr: true,
		},
		{
			name:    "valid spec",
			spec:    SourceSpec{ID: "test", Name: "Test", Category: SourceCategoryScanner, Priority: SourcePriorityP1},
			wantErr: false,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			err := m.DeploySource(context.Background(), tt.spec, SourceConfig{})
...
func TestSourceManagerModule_StatePersistence(t *testing.T) {
	...
	m1 := NewSourceManagerModule()
	m1.stateDir = dir

	spec := SourceSpec{ID: "trivy", Name: "Trivy", Category: SourceCategoryScanner, Priority: SourcePriorityP0, Version: "1.0"}
	m1.DeploySource(context.Background(), spec, SourceConfig{})
	m1.EnableSource(context.Background(), "trivy")
	m1.saveState()

	m2 := NewSourceManagerModule()
	m2.stateDir = dir
	m2.specs = make(map[string]SourceSpec)
	m2.statuses = make(map[string]SourceStatus)
	m2.configs = make(map[string]SourceConfig)
	m2.loadState()
...
func TestSourceManagerModule_DependencyCheck(t *testing.T) {
	...
	parentSpec := SourceSpec{ID: "netbox", Name: "NetBox", Category: SourceCategoryManagement, Priority: SourcePriorityP0, Version: "3.5"}
	m.DeploySource(context.Background(), parentSpec, SourceConfig{})
	m.EnableSource(context.Background(), "netbox")

	childSpec := SourceSpec{
		ID:        "wazuh_siem",
		Name:      "Wazuh SIEM",
	...
	err := m.DeploySource(context.Background(), childSpec, SourceConfig{})
	if err != nil {
		t.Fatalf("DeploySource with satisfied dependency failed: %v", err)
	}

	orphanSpec := SourceSpec{
		ID:        "broken",
		Name:      "Broken",
		Category:  SourceCategoryScanner,
		Priority:  SourcePriorityP1,
	...
	err = m.DeploySource(context.Background(), orphanSpec, SourceConfig{})
	if err == nil {
...
func TestSourceManagerModule_UninstallWithDependent(t *testing.T) {
	...
	m.DeploySource(context.Background(), childSpec, SourceConfig{})
...
func TestSourceManagerModule_SaveStateNoDir(t *testing.T) {
	...
	m.DeploySource(context.Background(), SourceSpec{ID: "test", Name: "Test", Category: SourceCategoryScanner, Priority: SourcePriorityP0, Version: "1.0"}, SourceConfig{})
...
func TestSourceManagerModule_EnableNotInstalled(t *testing.T) {
	...
	m.specs["ghost"] = SourceSpec{ID: "ghost", Name: "Ghost"}
	m.statuses["ghost"] = SourceStatus{ID: "ghost", State: SourceStateNotInstalled}
...
func TestSourceManagerModule_ConfigureNonExistent(t *testing.T) {
	...
	err := m.ConfigureSource(context.Background(), "nonexistent", SourceConfig{Settings: map[string]string{"key": "val"}})
	if err == nil {
