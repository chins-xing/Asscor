...
func TestSPCClearCache(t *testing.T) {
	...
	spc.AddCVE(SPCCVEScore{CVEID: "CVE-TEST-01", CVSS: 5.0, DatePublished: time.Now()})
	spc.AddCVE(SPCCVEScore{CVEID: "CVE-TEST-02", CVSS: 7.0, DatePublished: time.Now()})
...
func TestSPCClassifyAction(t *testing.T) {
	...
	for _, tt := range tests {
		got := classifyAction(tt.pscore).String()
		if got != tt.expected {
