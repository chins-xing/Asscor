...
func NewExtensionManager(cfg ManagerConfig) *ExtensionManager {
	...
	log.Printf("[extmgr] manager initialized with %d repo(s), dir=%s", len(mgr.repos), cfg.ExtensionsDir)
	return mgr
}
...
func (m *ExtensionManager) InstallFromSpec(spec ExtensionSpec) error {
	...
	if m.config.AutoEnable {
		if err := m.lifecycle.Enable(spec.ID); err != nil {
			log.Printf("[extmgr] auto-enable %s failed: %v", spec.ID, err)
		}
	}
...
func (m *ExtensionManager) AddRepository(name, url string) {
	...
	log.Printf("[extmgr] added repository %s (%s)", name, url)
}
...
func (m *ExtensionManager) RemoveRepository(name string) {
	...
	for i, r := range m.repos {
		if r.Name == name {
			...
			log.Printf("[extmgr] removed repository %s", name)
			return
		}
	}
}
...
func (m *ExtensionManager) RefreshRepositories(ctx context.Context) error {
	...
	for _, repo := range repos {
		log.Printf("[extmgr] refreshing repository %s (%s)...", repo.Name, repo.URL)
	}
	return nil
}
...
func (m *ExtensionManager) onExtensionInstalled(spec ExtensionSpec) {
	...
	case ExtTypeDomain:
		...
		log.Printf("[extmgr] registered domain %s from extension", spec.ID)
	case ExtTypeEdgeFactor:
		...
		log.Printf("[extmgr] registered edge factor %s from extension", spec.ID)
	case ExtTypeHook:
		if a != nil {
			...
			log.Printf("[extmgr] registered hook %s from extension", spec.ID)
		}
	}
}
...
func (m *ExtensionManager) onExtensionDisabled(spec ExtensionSpec) {
	...
	case ExtTypeCheckModule:
		...
		log.Printf("[extmgr] unregistered %d checks from extension %s", count, spec.ID)
	case ExtTypeDomain:
...
func (m *ExtensionManager) registerCheckModule(spec ExtensionSpec) {
	...
	if _, err := os.Stat(checkDir); os.IsNotExist(err) {
		log.Printf("[extmgr] no checks directory in extension %s", spec.ID)
		return
	}

	entries, err := os.ReadDir(checkDir)
	if err != nil {
		log.Printf("[extmgr] failed to read checks dir for %s: %v", spec.ID, err)
		return
	}
	...
	for _, entry := range entries {
		...
		for _, f := range files {
			...
			log.Printf("[extmgr] found check source in extension %s: %s/%s", spec.ID, entry.Name(), f.Name())
		}
	}
}
