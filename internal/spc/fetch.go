...
func (m *SPCModule) FetchFromAllSources() []SPCFetchResult {
	...
	resultMISP := m.FetchFromMISP()
	results = append(results, resultMISP)
	if m.mispConfig.BaseURL != "" {
		overallBar.Increment()
	}
...
type PersistenceInterface interface {
	WriteCVECache(record CVECacheRecord)
}
	...
	for _, cve := range cves {
		if idx, exists := m.cveIndex[cve.CVEID]; exists {
			...
			if len(m.cveCache) >= m.maxCacheSize {
				logger.WithComponent("spc").Warn("CVE cache reached max size")
				break
			}
...
func (m *SPCModule) fetchNVDWindow(client *http.Client, baseURL, apiKey string, startDate, endDate time.Time, useLastMod, noRejected bool, startIdx, retryCount *int, maxRetries int, prog *common.ProgressBar) ([]SPCCVEScore, error) {
	var allCVEs []SPCCVEScore
