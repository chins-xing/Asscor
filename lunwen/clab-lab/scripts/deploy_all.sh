...
sleep 8
docker exec asc-asscor-edge0 bash -c "grep -c 'ASSCOR .Kernel started' /var/log/asscor-kernel.log"
echo "=== 2. 导出新 CA + 签发 host1-8 ==="
...
docker exec asc-asscor-edge0 bash -c "cat /var/lib/asscor/heartbeat_identity.json | python3 -m json.tool 2>/dev/null || cat /var/lib/asscor/heartbeat_identity.json; echo"
echo "=== 5. 心跳 ==="
