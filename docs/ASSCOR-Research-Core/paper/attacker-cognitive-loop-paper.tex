...
\textbf{Invitation to the community.} We explicitly invite researchers working
on attacker modeling, deception, game theory, decision theory (I-POMDP\textsuperscript{2}),
and learning-based prediction to propose more principled models for any of the
three engine seams (state estimation, action prediction, engagement
selection). The loop architecture is model-agnostic (\S\ref{sec:replacement}):
a better model can be dropped into the corresponding interface without
changing the graph, the controller, or the closed-loop contract. The reference
implementation is open source (Apache-2.0) and the lab environment is
reproducible. Please reach out via the contact address above.
}}
\end{center}
...
The rule-based engines presented here are explicitly \textbf{first
approximations}, not claims of optimality. We believe the most valuable next
step is not tuning these hand-crafted weights but replacing the approximations
with more principled models. The architecture is explicitly designed so that
any component can be swapped at the three seams defined in
\S\ref{sec:replacement} --- we invite the community to propose and evaluate
better models within this harness. We therefore \textbf{openly invite} the
research community to:
\begin{itemize}
\item propose \textbf{more principled state-estimation models} (e.g.,
      I-POMDP\textsuperscript{2} solvers, Bayesian belief updates) to
      replace our rule-based $\mathit{StateUpdater}$;
\item propose \textbf{calibrated prediction models} (e.g., learning-based
      action distribution predictors) to replace our rule-based
      $\mathit{ActionPredictor}$;
\item propose \textbf{better engagement-selection criteria} (e.g.,
      information-theoretic optimal experimental design, game-theoretic
      signal models) to replace or refine our utility function;
\item challenge our open research questions Q1--Q10 with data, experiments,
      or theory.
\end{itemize}
Because every engine seam is a narrow interface
(\S\ref{sec:replacement}), a proposed model can be integrated and evaluated
against the reference implementation with minimal effort. The reference
implementation is open source under Apache-2.0, and the multi-host lab
environment is reproducible.

Contact: \texttt{chins-xing@proton.me} --- we will treat every message as a
