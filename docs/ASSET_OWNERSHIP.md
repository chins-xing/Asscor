...
| 输出 | 含义 | 处置 |
|---|---|---|
| **绿 exit=0** | 底座路径两侧**逐字节相同**（清单为空或全部 stale） | 可以继续 |
| **黄 exit=3** | 清单外逐字节一致；清单内 14 条有意差异**均与登记一致** | 正常状态 |
| **红 exit=1** | 出现**清单之外**的漂移 | 要么把漂移**消除**（按规则② 上 main），要么**登记**（按规则④ 写理由与引入提交） |

**当前实测状态（2026-09-13）**：带清单对照真 `main`(`fdb116e`) ⇒ `清单条目=14 active=14 stale=0`、**清单外未解释 = 0** ⇒ **黄 exit=3**。
（不带清单对照分叉前的 `df8342e` ⇒ 22 个漂移、其中 8 个未解释 ⇒ 红；那 8 个正是已按规则② 上 main 的 A 类产品侧修复。）
