...
| 注意点 | 说明 |
|------|------|
| ?? **边缘因子 TriggerCheck 依赖** | 替换检查项需同步更新 `TriggerCheck` 映射 |
| ?? **ATT&CK 技术映射** | 需在 ATTACKModule 中同步更新 |
| ?? **等保条款标注** | 替换后需标注正确的等保条款编号 |
...
| 事件总线话题 | 13 |
| 总线订阅关系 | 6 |
| 扩展点总数 | 76（内核 6 + 业务模块 70） |
| 全部可替换 | ? |

---

## 11. 外部扩展模块与扩展包（v0.2.2+）

ASSCOR v0.2.2 引入了**外部扩展体系**——与内核集成的 ExtensionManager (extmgr) 互补的、独立编译的模块化扩展系统。核心设计原则：**外部扩展不修改内核代码，通过 Go import 和 Extension Point 系统挂载**。
