...
## 11. 外部扩展模块与扩展包（v0.2.3+）

ASSCOR v0.2.3 引入了**外部扩展体系**——与内核集成的 ExtensionManager (extmgr) 互补的、独立编译的模块化扩展系统。核心设计原则：**外部扩展不修改内核代码，通过 Go import 和 Extension Point 系统挂载**。
