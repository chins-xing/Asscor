...
> 版本：v0.2.3 | SSAM 2.0 | 最后更新：2026-08-12
...
ASSCOR v0.2.3 集成 MITRE ATT&CK V19 框架，作为 μKernel 插件（`attck`，优先级 21，版本 1.0.0）运行。模块提供从检测、情报、仿真到评估的完整威胁分析能力链，并在此基础上扩展 APT 攻击分析与检测增强子模块。
...
| 版本 | 日期 | 主要变更 |
|------|------|----------|
| v0.2.3 | 2026-08-12 | 技术债务清偿80项(87→7, 92%): P0全清零(19/19)、内核17接口/类型文件拆分、25测试文件/222用例/5Benchmark、引擎钩子↔扩展点桥接(8阶段)、SemVer统一、extmgr桥接(9类型)、引擎适配器归位、ATT&CK build-tag分离、10+nil-guard修复、TLS消重、系统d消重、pkgmgr融合、多算法编排可选化、扩展包管理体系(package.json+pkgmgr+SCHEMA) |
