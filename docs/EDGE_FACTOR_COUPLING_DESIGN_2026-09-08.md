...
- **阈值敏感性（含一处实测纠正）**：记录里的 `threshold` 来自部署配置（冒烟为 80.0，全部记录判"不可接受"）。**实测纠正**：该退化并**不**表现为"误阻断率 → 100 %" —— 冒烟三条记录**全部** `compromised=true`（**没有负类**，误阻断率分母为零），故漏判率与误阻断率**都**是 0.000，退化表现为**"本次比较无区分力：未选模"**。更值得注意的是：在该数据集上按漏判率排序会**奖励"全都阻断"**的策略（`-threshold 60` 的敏感性行给出 legacy 漏判率 0、其余候选 1.000 ⇒ 工具"选定 legacy"）—— 这正是**缺少负类样本**时的退化最优，也是"标签必须有方差"的又一实证理由。`cmd/edgecompare -threshold` 提供**显式**敏感性覆盖（`0` = 用记录自带；>0 覆盖并在报告头/stderr 标注为敏感性值），**不得**把它当默认路径。
