...
ASSCOR v0.2.0 在 MITRE ATT&CK V19 框架基础上，构建了完整的 APT 攻击分析与检测增强模块。该模块通过四大核心能力——攻击链重构、行为检测、APT 归因和威胁狩猎——实现了从"单点告警"到"攻击链可视化"的范式跃迁。本白皮书系统阐述各核心能力的设计原理、算法实现、数据模型和工程实践，并说明其与 SSAM 评估体系的协同机制。
