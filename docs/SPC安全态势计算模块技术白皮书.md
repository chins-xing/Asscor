...
> **版本：** v2.0 | **适用：** ASSCOR v0.2.3 / SSAM 2.0 | **日期：** 2026-06-28
...
> ?? **重要声明**：SPC 模块的验证逻辑基于 **CPE（Common Platform Enumeration）字符串匹配**——即将已安装软件包的名称/版本与 CVE 数据库中记录的受影响产品版本进行交叉比对。它**不执行**以下任何深度验证：
>
...
| 匹配策略 | 全量 `strings.Contains` | 优先 map 查找，fallback 子串 | O(n2) → O(n) |
...
> **说明：** SPC 是 SSAM 2.0 的核心组件，当前版本 v2.0。ASSCOR 是实现 SSAM 的开源项目框架，当前版本 v0.2.3。两者版本号独立演进。
>
